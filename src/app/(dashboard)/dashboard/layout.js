...
export default function DashboardLayout({ children }) {
  ...
  const inactiveCls = 'text-[#7A7A7A] hover:bg-[#5A3045]/5 hover:text-[#5A3045] border-r-4 border-r-transparent rounded-2xl';
  ...
  return (
    <div className="flex min-h-screen bg-[#FAF7F3] text-[#1F1F1F] font-sans relative overflow-x-hidden">
      ...
      {/* ── Sidebar ─────────────────────────────── */}
      <aside className={`fixed top-4 bottom-4 w-64 bg-white/95 border border-[rgba(90,48,69,0.08)] rounded-[24px] flex flex-col p-4 shadow-sm z-50 transition-all duration-300 lg:left-4 ${sidebarOpen ? 'left-4' : '-left-80 lg:left-4'}`}>
        ...
        <nav className="flex-1 px-1 py-2 space-y-1.5 overflow-y-auto">
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-3 mb-2">Main Menu</p>
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                ...
                <span className={`transition-transform duration-200 group-hover:scale-110 ${active ? 'text-[#D4A45B]' : 'text-[#7A7A7A] group-hover:text-[#5A3045]'}`}>{item.icon}</span>
          ...
          {isOwner && (
            <>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-3 mt-6 mb-2">Organization</p>
              {ownerItems.map((item) => {
                const active = pathname === item.href;
                return (
                  <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
        ...
        <div className="pt-4 border-t border-[rgba(90,48,69,0.08)] space-y-2">
          <div className="flex items-center space-x-2.5 p-2 rounded-2xl bg-[#FAF7F3] border border-[rgba(90,48,69,0.06)]">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="text-xs font-bold text-[#1F1F1F] truncate">{user.name}</h4>
              <p className="text-[9px] text-[#7A7A7A] flex items-center space-x-1">
      ...
      <div className="pl-4 lg:pl-[296px] pr-4 py-4 flex flex-col flex-1 min-w-0 relative z-10">
        ...
        {/* Premium Navbar */}
        <PremiumNavbar user={user} setSidebarOpen={setSidebarOpen} handleLogout={handleLogout} />
...
───────────────────────────────────────────────── */
function PremiumNavbar({ user, setSidebarOpen, handleLogout }) {
  ...
  const iconBtn = 'w-9 h-9 rounded-xl bg-[#FAF7F3] border border-[rgba(90,48,69,0.08)] hover:border-[#D4A45B]/40 hover:bg-white flex items-center justify-center text-[#7A7A7A] hover:text-[#5A3045] transition-all';

  return (
    <header className="w-full sticky top-4 z-30">
      <div className="bg-white/80 backdrop-blur-xl border border-[rgba(90,48,69,0.09)] rounded-[28px] shadow-sm shadow-[#5A3045]/5 px-5 py-3.5 flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="hidden xl:block">
            <p className="text-sm font-black text-[#1F1F1F] leading-tight whitespace-nowrap">
              {greeting}, {firstName} {emoji}
            </p>
            <p className="text-[10px] text-[#8A7A6A] font-medium mt-0.5 whitespace-nowrap">
        ...
        <div className="flex-1 max-w-xl mx-auto">
          <motion.div
            ...
            className={`relative flex items-center bg-[#FAF7F3] border rounded-2xl px-4 py-2.5 transition-all duration-300 ${
              searchFocused
                ? 'border-[#D4A45B]/60 shadow-md shadow-[#D4A45B]/10 bg-white'
                : 'border-[rgba(90,48,69,0.08)] hover:border-[rgba(90,48,69,0.15)]'
            }`}
          >
            <Search size={14} className={`shrink-0 mr-3 transition-colors ${searchFocused ? 'text-[#5A3045]' : 'text-[#B0A090]'}`} />
            <input
              type="text"
              value={searchValue}
              ...
              className="flex-1 bg-transparent text-xs font-medium text-[#1F1F1F] placeholder-[#B0A090] focus:outline-none"
            />
            {!searchFocused && !searchValue && (
              <span className="shrink-0 ml-2 text-[9px] font-black text-[#B0A090] bg-[#F0E8DE] border border-[#E0D0C0] px-2 py-0.5 rounded-md tracking-wider">⌘K</span>
            )}
            {searchValue && (
              <button onClick={() => setSearchValue('')} className="shrink-0 ml-2 text-[#B0A090] hover:text-[#5A3045] transition-colors">
                <X size={12} />
              </button>
            )}
          </motion.div>
        ...
        <div className="flex items-center gap-2 shrink-0">
                ...
                >
                  ...
                  {quickActions.map((a) => (
                    <Link key={a.href} href={a.href} onClick={() => setQuickOpen(false)}
                      ...
                      <span className="text-xs font-semibold text-[#1F1F1F] group-hover:text-[#5A3045]">{a.label}</span>
                    </Link>
                  ))}
                </motion.div>
              )}
              ...
              {notifOpen && (
                ...
                >
                  <div className="flex items-center justify-between px-1 pb-2 border-b border-[#F0E8E0] mb-2">
                    <span className="text-xs font-black text-[#1F1F1F]">Notifications</span>
                    <span className="text-[9px] font-black text-[#D4A45B] bg-[#D4A45B]/10 px-2 py-0.5 rounded-full">{notifications.length} new</span>
                  </div>
                  <div className="space-y-0.5">
                    {notifications.map((n, i) => (
                      <div key={i} className="flex items-start space-x-3 p-2.5 rounded-xl hover:bg-[#FAF7F3] transition-colors cursor-pointer group">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-[#1F1F1F] group-hover:text-[#5A3045] transition-colors">{n.title}</p>
          ...
          <motion.div
            ...
            className="hidden md:flex items-center space-x-2 bg-[#FAF7F3] border border-[rgba(90,48,69,0.08)] hover:border-[#D4A45B]/40 hover:bg-white px-3 py-2 rounded-xl cursor-pointer transition-all group"
          >
            ...
            <span className="text-[10px] font-black text-[#1F1F1F] max-w-[80px] truncate group-hover:text-[#5A3045] transition-colors">
              {user?.organizationId ? 'Team Workspace' : 'Personal'}
            </span>
            <svg width="10" height="10" viewBox="0 0 10 10" fill="none" className="text-[#B0A090] shrink-0">
                  ...
                  <div className="p-4 bg-gradient-to-br from-[#FAF7F3] to-[#F5EDE4] border-b border-[#F0E8E0]">
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-black text-[#1F1F1F] truncate">{user?.name}</p>
                  ...
                  <div className="p-2">
                    {[
                      ...
                      <Link key={item.label} href={item.href} onClick={() => setProfileOpen(false)}
                        ...
                        <span className="text-xs font-semibold text-[#1F1F1F] group-hover:text-[#5A3045] transition-colors">{item.label}</span>
                      </Link>
                    ))}
